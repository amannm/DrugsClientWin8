...
namespace DrugsX {
    ...
    sealed partial class App : Application {
        ...
        public async void searchPane_QuerySubmitted(SearchPane sender, SearchPaneQuerySubmittedEventArgs args) {
            ...
            var coll = await Service.FindTermsAsync(args.QueryText);
            frame.Navigate(typeof(SearchResultsPage), coll);
        }
        ...
        protected async override void OnSearchActivated(Windows.ApplicationModel.Activation.SearchActivatedEventArgs args) {
            ...
            if (frame == null) {
                ...
                frame = new Frame();
                DrugsX.Common.SuspensionManager.RegisterFrame(frame, "AppFrame");

                if (args.PreviousExecutionState == ApplicationExecutionState.Terminated) {
                    // Restore the saved session state only when appropriate
                    try {
                        await DrugsX.Common.SuspensionManager.RestoreAsync();
                    } catch (DrugsX.Common.SuspensionManagerException) {
                        //Something went wrong restoring state.
                        //Assume there is no state and continue
                    }
                }
            }
            var results = await Service.FindTermsAsync(args.QueryText);
